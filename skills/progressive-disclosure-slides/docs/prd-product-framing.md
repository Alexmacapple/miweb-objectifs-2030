...
**Décision proposée** : atelier guidé PDS avec façade studio minimale
...
Le mécanisme à préserver est le substrat PDS : partir d'une scène, montrer une transformation visible, limiter le texte, stabiliser un masque commun et exiger une preuve de sortie. Le moteur image devient un adaptateur interchangeable ; le skill reste responsable de la narration, du contrat de rendu et de la validation.
...
4. Albert OSS-120, ou un autre modèle texte, peut aider à rédiger le storyboard et les prompts.
5. Un adaptateur de rendu reçoit les prompts et produit des images, ou marque le rendu non disponible.
6. Le système collecte chemin image, ratio, prompt, statut et défauts.
7. Une planche contact ou un équivalent permet de contrôler la cohérence de série.
8. L'utilisateur récupère un dossier projet complet : storyboard, prompts, reçus, images ou statuts `NOT VERIFIED`.
...
Un nettoyage naïf peut supprimer ce qui fait la valeur du skill : la preuve de génération, l'inspection, le reçu, la contact sheet et les contraintes PDS. À l'inverse, transformer tout de suite l'idée en studio complet créerait trop de surfaces non vérifiées.

## Solution

Retenir une architecture en deux couches :

- **Noyau PDS** : skill générique, compact, indépendant du rendu, avec invariants bloquants.
- **Façade studio légère** : prototype statique qui montre une navigation projets/sources/styles/moteurs/exports, mais n'implémente réellement que le flux guidé Source -> PDS -> Prompts -> Rendu -> Contrôle.

Le futur livrable dans `outputs/skill/` doit contenir :
...
- `prototype/index.html` : maquette statique de l'atelier guidé avec façade studio.
...
### Option B retenue : Atelier guidé avec façade studio minimale

**Avantages** :
- Conserve le coeur PDS.
- Donne une interface compréhensible pour des non-spécialistes.
- Prépare l'évolution vers plusieurs moteurs sans les implémenter trop tôt.
- Évite de présenter Albert comme moteur image.

**Inconvénients** :
- Risque d'illusion produit si la façade paraît fonctionnelle.
- Demande de bien marquer les zones non implémentées.
...
**Inconvénients** :
- Trop large pour un prototype.
- Masque le vrai test : le noyau PDS survit-il à la généricité ?
- Implique backend, persistance, comptes et connecteurs réels.

## Décision

Retenir **Option B** : atelier guidé comme workflow réel, façade studio seulement comme orientation visuelle et structure de navigation. Le prototype ne doit pas faire croire à une gestion de projet complète.
...
- Le prototype MUST montrer le flux Source -> PDS -> Prompts -> Rendu -> Contrôle.
- Le prototype MUST signaler clairement que les onglets studio avancés sont hors MVP.
...
- Pas d'intégration Albert fonctionnelle dans le prototype.
- Pas de génération d'image réelle dans le prototype.
- Pas de packaging PPTX dans le MVP.

## Plan d'implémentation

1. Créer `outputs/skill/pds-slide-generator/`.
...
4. Rédiger `references/renderer-contract.md`.
5. Rédiger `references/text-model-role.md`.
6. Créer `prototype/index.html` comme prototype statique utilisable dans navigateur.
7. Contrôler les accents sur les Markdown.
8. Vérifier que les fichiers existent et que le prototype s'ouvre localement.
...
- Le prototype contient exactement le flux principal en 5 étapes.
- Les Markdown passent `bash scripts/check-accents.sh <fichier>`.
- Le dossier final est entièrement sous `outputs/skill/`.
...
- Le LLM peut ajouter du texte parasite dans les prompts ou les maquettes.
...
- un avertissement explicite dans le prototype : rendu réel non branché ;
- une règle stricte : si aucun fichier image n'existe, ne jamais simuler une réussite ;
- une preuve minimale : liste des fichiers, contrôle accents, ouverture du HTML.

## Passage PDG

**Déclenchement** : oui, car ce PRD est une spécification qui sera exécutée par un agent.

**Skill invocation pass** : `prd` utilisé pour le format de cadrage ; `progressive-disclosure-guard` utilisé pour éviter un handoff ambigu. Le workflow PRD canonique du workspace n'est pas complété volontairement : ce document est un brouillon de cadrage dans `outputs/skill/`.
...
**Connus connus** : le MVP doit rester statique, générique, centré PDS, sous `outputs/skill/`.
...
**Comportement existant à préserver** : storyboard concret, preuve de sortie, inspection, refus de réussite simulée.

**Raccourcis interdits** : supprimer le reçu, simuler des images, présenter Albert comme moteur image sans preuve, transformer le prototype en app complète.
